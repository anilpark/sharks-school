...
<section id="reviews" class="w-full section-pad blue-pattern section-bleed reviews-bleed">
					...
					>
						<div class="relative h-full w-full overflow-hidden rounded-[12px] bg-[#d9d9d9]">
							<img
								...
								decoding="async"
							/>
							<div class="absolute inset-0 bg-black/15 transition-colors duration-300 group-hover:bg-black/25"></div>
							<span class="absolute inset-0 flex items-center justify-center">
								<span class="flex h-[52px] w-[52px] items-center justify-center rounded-full bg-white/95 shadow-[0_8px_24px_rgba(0,0,0,0.22)] transition-transform duration-300 group-hover:scale-110">
			...
			{#each mobileRows as row}
				<div class="flex snap-x snap-mandatory gap-3 overflow-x-auto px-6 max-[380px]:px-4 pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
					{#each row as review, index (review.id)}
						<div
							class="flex h-[130px] shrink-0 snap-center flex-col rounded-[14px] bg-white p-3 overflow-hidden"
							class:w-[296px]={index % 2 === 0}
							class:w-[272px]={index % 2 !== 0}
						>
							<div class="flex items-start gap-2">
								...
								<div class="flex min-w-0 flex-col gap-1">
									...
									<p class="text-[12px] leading-[1.25] break-words text-[#262626] [overflow-wrap:anywhere] overflow-hidden [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:4]">
										{review.text}
									</p>
								</div>
							</div>
